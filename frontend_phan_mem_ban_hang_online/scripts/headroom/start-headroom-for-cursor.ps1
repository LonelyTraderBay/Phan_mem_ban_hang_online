...
$mode = Get-HeadroomProxyMode -Profile balanced
$label = Get-HeadroomProfileLabel -Profile balanced

Write-Host "Starting Headroom proxy at http://localhost:8787 with $label ..."
Write-Host "For Cursor subscription: use run-cursor-through-headroom.ps1 for MCP + learn."
