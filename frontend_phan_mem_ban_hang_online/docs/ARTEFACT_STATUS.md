...
| `asyncapi/tenant-events.yaml` | ✅ Present — synced from backend |
| `asyncapi/ops-events.yaml` | ✅ Present, but a deliberate **stub** — no ops-scoped events exist in the backend contract yet |
...
- `ux/handoff-checklist.md` — ✅ present; F01 auth + settings rows **READY-MOCK** (Human Owner
  approved 2026-07-21).
...
| Artefact | Status |
|---|---|
| `F01-preflight.md` | ✅ Present (2026-07-21) — FE-F01-001…006 DoR; READY-MOCK + OIDC ops; awaiting `contracts:sync` after BE GAP-009 |
...
| Auth strategy | **Resolved** — ADR-FE-013 OIDC + BFF cookie (HO 2026-07-21). Internal-credentials login UI out of scope for Web Admin. BE GAP-009 Closed. | FE must `contracts:sync` after BE commit |
