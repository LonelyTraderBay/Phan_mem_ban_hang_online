# Design spec: `/channels` — Kênh / Sức khỏe kênh

**Status:** READY-MOCK — HO defaults policy C (enterprise freeze W6) 2026-07-22
**Version:** v1 — 2026-07-22
**Author:** Design AI Agent
**Route:** `/channels, /channels/:channelId/health`
**Required permission(s):** channel.read; channel.connect / ops.channel.manage as gated
**App:** web-admin
...
```text
LIST: channel accounts + health StatusBadge
...
- Credentials never shown in clear text. Entitlement meter.channel_accounts applies.
- Sample chrome: `[DRAFT COPY]` titles as in layout sketch.

### Empty

- EmptyState: [DRAFT COPY] Chưa kết nối kênh nào. Kết nối để nhận hội thoại.
...
- Missing **channel.read**: full-page ForbiddenState via PermissionGate.
- Copy: `[DRAFT COPY] Bạn không có quyền xem kênh.`

### Conflict

- Token refresh conflict → reconnect CTA.
...
| Health chart | **MISSING COMPONENT** | — |
| Provider logo row | **MISSING COMPONENT** | — |
...
Connect → provider redirect → callback → list refresh. Health auto-refresh.
