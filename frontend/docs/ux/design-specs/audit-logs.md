...
```text
Cross-tenant audit search (platform)
...
- Nullable-tenant audit rows allowed for global actions.
- Sample chrome: `[DRAFT COPY]` titles as in layout sketch.

### Empty

- EmptyState: [DRAFT COPY] Không có sự kiện khớp bộ lọc.
...
- Missing **ops audit**: full-page ForbiddenState via PermissionGate.
- Copy: `[DRAFT COPY] Bạn không có quyền xem audit nền tảng.`
...
| Cross-tenant audit table | **MISSING COMPONENT** | — |
...
Search/filter → export if allowed.
