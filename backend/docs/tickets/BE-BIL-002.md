---
ticket_id: BE-BIL-002
title: Usage event metering/idempotency/reconciliation
...
Usage event metering/idempotency/reconciliation.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/billing/`.

# Actor and use case

Actors and flows for domain **BIL** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P10).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for BIL; implement only operations this ticket owns.
