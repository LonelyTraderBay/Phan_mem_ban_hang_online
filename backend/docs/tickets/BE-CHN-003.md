---
ticket_id: BE-CHN-003
title: OAuth state/PKCE where applicable/callback/token vault
...
OAuth state/PKCE where applicable/callback/token vault.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/channel/`.

# Actor and use case

Actors and flows for domain **CHN** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P5).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for CHN; implement only operations this ticket owns.
