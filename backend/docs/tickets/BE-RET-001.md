---
ticket_id: BE-RET-001
title: Return/receive/inspect/restock/refund flow
...
Return/receive/inspect/restock/refund flow.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/fulfillment/`.

# Actor and use case

Actors and flows for domain **RET** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P7).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for RET; implement only operations this ticket owns.
