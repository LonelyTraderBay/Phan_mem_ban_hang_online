...
Warehouse/balance/movement/reservation schema + constraints/RLS.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/inventory/`.

# Actor and use case

Actors and flows for domain **INV** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P4).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for INV; implement only operations this ticket owns.
