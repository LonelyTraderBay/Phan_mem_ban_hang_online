---
ticket_id: BE-PAY-003
title: Provider callback adapter baseline when selected
...
Provider callback adapter baseline when selected.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/payment/`.

# Actor and use case

Actors and flows for domain **PAY** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P7).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for PAY; implement only operations this ticket owns.
