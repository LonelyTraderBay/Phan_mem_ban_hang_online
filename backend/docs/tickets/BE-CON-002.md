---
ticket_id: BE-CON-002
title: Inbound normalized event -> identity/conversation/message upsert
...
Inbound normalized event -> identity/conversation/message upsert.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/conversation/`.

# Actor and use case

Actors and flows for domain **CON** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P6).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for CON; implement only operations this ticket owns.
