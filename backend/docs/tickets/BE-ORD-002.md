---
ticket_id: BE-ORD-002
title: Deterministic calculation/rounding/discount/tax rules
...
Deterministic calculation/rounding/discount/tax rules.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/order/`.

# Actor and use case

Actors and flows for domain **ORD** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P7).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for ORD; implement only operations this ticket owns.
