---
ticket_id: BE-DAT-001
title: Business event schema/taxonomy completeness review
...
Business event schema/taxonomy completeness review.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/analytics/`.

# Actor and use case

Actors and flows for domain **DAT** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P9).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for DAT; implement only operations this ticket owns.
