---
ticket_id: BE-DSK-005
title: Crash telemetry ingestion if not vendor SDK
...
Crash telemetry ingestion if not vendor SDK.
...
Actors and flows for domain **DSK** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P10).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for DSK; implement only operations this ticket owns.
