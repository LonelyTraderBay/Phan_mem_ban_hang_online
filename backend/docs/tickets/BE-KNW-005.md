---
ticket_id: BE-KNW-005
title: Published/effective-only tenant retrieval API
...
Published/effective-only tenant retrieval API.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/knowledge/`.

# Actor and use case

Actors and flows for domain **KNW** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P4).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for KNW; implement only operations this ticket owns.
