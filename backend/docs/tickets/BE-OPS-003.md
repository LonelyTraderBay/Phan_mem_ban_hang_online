---
ticket_id: BE-OPS-003
title: Support access grants and scoped service token
...
Support access grants and scoped service token.

Deliverable / details from backlog: (none — expand from blueprint + contracts before coding).

Primary paths: `modules/operations/`.

# Actor and use case

Actors and flows for domain **OPS** as defined in the enterprise blueprint and frozen OpenAPI/AsyncAPI contracts for this phase (P10).
...
- OpenAPI operation/schema: slice with `pnpm agent:contract-slice` for OPS; implement only operations this ticket owns.
