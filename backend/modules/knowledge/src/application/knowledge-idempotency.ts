...
export function knowledgeIdempotencyContext(
  tenantId: string,
  actorId: string
...
export async function runKnowledgeIdempotent<TResult>(options: {
  ...
  const ctx = knowledgeIdempotencyContext(options.tenantId, options.actorId);
  const idemReq = {
    scope: options.scope,
    key: options.key,
  ...
  try {
    ...
    if (reserve.outcome === "replay") {
      ...
      throw new KnowledgeError("Idempotent replay missing result.", "RESOURCE_NOT_FOUND");
    }
    acquired = true;
    const result = await options.execute();
    ...
    if (error instanceof IdempotencyInProgressError) {
      throw new KnowledgeError("Idempotency key is still processing.", "VALIDATION_FAILED");
    }
    ...
    if (acquired) {
      const retryable = !(error instanceof KnowledgeError);
